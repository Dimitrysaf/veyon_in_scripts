        ...
        for chunk in response.iter_content(chunk_size=block_size):
            if chunk:
                ...
                if total_size > 0:
                    ...
                    print(
                        f"\rDownloading: {percent}% ({mb_downloaded:.1f} MB / {mb_total:.1f} MB)",
                        end="",
                    )
    ...
    try:
        ...
        if keys_destination.exists():
            try:
                ...
                # Only try elevation if we're NOT already admin
                if not is_admin():
                    logger.info("Attempting to fix permissions with UAC...")
                    if not remove_directory_elevated(keys_destination):
                        logger.error(
                            "Failed to remove old keys directory even with elevation"
                        )
                        return False
                else:
                    logger.error("Running as admin but still got permission denied")
                    return False
        ...
        logger.info(
            f"Successfully copied {file_count} file(s) from Veyon keys directory"
        )
...
def pin_to_taskbar(app_path):
    """Pin an application to the Windows taskbar"""
    logger = get_logger()

    if not Path(app_path).exists():
        logger.warning(f"Cannot pin to taskbar - file not found: {app_path}")
        return False

    logger.info(f"Attempting to pin to taskbar: {app_path}")

    try:
        # Method 1: Use PowerShell to pin to taskbar
        ps_script = f"""
$shell = New-Object -ComObject Shell.Application
$folder = $shell.Namespace((Get-Item "{app_path}").DirectoryName)
$item = $folder.ParseName((Get-Item "{app_path}").Name)
$verb = $item.Verbs() | Where-Object {{$_.Name -match 'Pin to taskbar'}}
if ($verb) {{
    $verb.DoIt()
    Write-Host "Pinned to taskbar"
}} else {{
    Write-Host "Pin verb not found"
}}
"""
        ...
        if result.returncode == 0:
            if "Pinned to taskbar" in result.stdout:
                logger.info("✓ Veyon Master pinned to taskbar")
                return True
            else:
                logger.warning("Pin to taskbar verb not found (may be Windows 11)")

        # Method 2: Fallback - Create taskbar shortcut manually (Windows 11)
        logger.info("Trying Windows 11 method - creating taskbar shortcut...")

        # Get user's AppData path for taskbar shortcuts
        ...
        if not taskbar_path.exists():
            logger.warning(f"Taskbar shortcuts folder not found: {taskbar_path}")
            return False

        # Create shortcut using PowerShell
        shortcut_path = taskbar_path / "Veyon Master.lnk"
        ps_shortcut = f"""
$WshShell = New-Object -ComObject WScript.Shell
$Shortcut = $WshShell.CreateShortcut("{shortcut_path}")
$Shortcut.TargetPath = "{app_path}"
$Shortcut.WorkingDirectory = (Get-Item "{app_path}").DirectoryName
$Shortcut.Description = "Veyon Master - Classroom Management"
$Shortcut.Save()
...
"""
        ...
        if result.returncode == 0 and shortcut_path.exists():
            logger.info("✓ Veyon Master shortcut added to taskbar folder")
            return True
        else:
            logger.warning("Could not create taskbar shortcut")
            return False

    except subprocess.TimeoutExpired:
        logger.error("Taskbar pin operation timed out")
        return False
    except Exception as e:
...
def install_teacher():
    ...
    try:
        ...
        for asset in assets:
            if (
                "win64" in asset["name"].lower()
                or "win64" in asset["browser_download_url"].lower()
            ):
                asset64 = asset
                break
        ...
        for asset in assets:
            ...
            if any(
                ...
                for keyword in ["sha256", "checksum", "hash", "sum"]
            ):
        ...
        if checksum_asset:
            ...
            try:
                checksum_response = requests.get(
                    checksum_asset["browser_download_url"], headers=headers
                )
                checksum_response.raise_for_status()
                checksum_text = checksum_response.text
                logger.debug(
                    f"Checksum file content:\n{checksum_text[:500]}"
                )  # Log first 500 chars
        ...
        if checksum:
            ...
            logger.warning(
                "Please verify this checksum manually if security is critical"
            )
        ...
        if sys.platform == "win32":
            ...
            if already_admin:
                # Already admin - run installer directly without UAC prompt
                ...
                # Not admin - need to elevate with UAC
                logger.warning("Not running as administrator!")
                logger.warning(
                    "The installer requires elevation. Attempting to elevate..."
                )
                ...
                try:
                    ...
                    sei_mask = (
                        shellcon.SEE_MASK_NOCLOSEPROCESS | shellcon.SEE_MASK_NO_CONSOLE
                    )
                    sei = shell.ShellExecuteEx(
                        fMask=sei_mask,
                        lpVerb="runas",
                    ...
                    if hProcess:
                        ...
                        if exit_code != 0:
                            logger.warning(
                                f"Installer returned non-zero exit code: {exit_code}"
                            )
                    else:
                        raise Exception(
                            "Failed to get process handle from elevated installer"
                        )
                    ...
                    result = ctypes.windll.shell32.ShellExecuteW(
                        None, "runas", str(temp_installer), "/S", None, 1
                    )
                    ...
                    logger.info("Installer launched with elevation (UAC prompted)")
                    logger.warning(
                        "Cannot track process without pywin32. Waiting 30 seconds..."
                    )
                    time.sleep(30)
                    exit_code = 0
        ...
        # Pin Veyon Master to taskbar
        logger.info("Pinning Veyon Master to taskbar...")

        # Find Veyon Master executable
        veyon_master_paths = [
            Path(os.environ.get("PROGRAMFILES", "C:/Program Files"))
            / "Veyon"
            / "veyon-master.exe",
            ...
            / "Veyon"
            / "veyon-master.exe",
            Path("C:/Program Files/Veyon/veyon-master.exe"),
        ]

        veyon_master = None
        for path in veyon_master_paths:
            if path.exists():
                veyon_master = path
                break

        if veyon_master:
            pin_success = pin_to_taskbar(veyon_master)
            if pin_success:
                logger.info("✓ Veyon Master is now pinned to taskbar for easy access")
            else:
                logger.info(
                    "Note: Could not auto-pin to taskbar. You can manually pin Veyon Master."
                )
        else:
            logger.warning("Veyon Master executable not found - cannot pin to taskbar")
