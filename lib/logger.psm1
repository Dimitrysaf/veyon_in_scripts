function Init-Logger {
    ...
    try {
        ...
        $file = Join-Path -Path $logDir -ChildPath ("menu_$time.log")
...
function Write-Log {
    param(
        [Parameter(Mandatory=$true)][ValidateSet('DEBUG','INFO','WARN','ERROR','FATAL')] [string] $Level,
    ...
    try {
        if (-not (Get-Variable -Name LoggerFile -Scope Global -ErrorAction SilentlyContinue)) {
            ...
            $global:LoggerFile = Join-Path -Path $fallbackDir -ChildPath 'menu_fallback.log'
        }
